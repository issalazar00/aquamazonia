...
class InformeRegistroController extends Controller
{
	...
	public function filtroRegistros(Request $request)
	{
		...
		$especies_siembra_controller = new EspeciesSiembraController;
		if (count($registros) > 0) {
			foreach ($registros as $registro) {
				$especies_siembra = $especies_siembra_controller->cantidadEspecieSiembra($registro->id_siembra, $registro->id_especie);

				$registro->mortalidad_general = $especies_siembra->mortalidad;
				$registro->biomasa_general = $especies_siembra->biomasa;
				$registro->salida_animales_general = $especies_siembra->cantidad + $registro->mortalidad_general;
				$registro->salida_animales_sin_mortalidad = $registro->cantidad;
				$registro->cantidad_actual = $registro->cantidad_inicial - $registro->salida_animales_general;
				$registro->biomasa_disponible = ((($registro->peso_actual) * ($registro->cantidad_actual)) / 1000);
				$registro->biomasa_inicial =  ((($registro->peso_inicial) * ($registro->cantidad_inicial)) / 1000);
